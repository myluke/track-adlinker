...
<main id="main" tabindex="-1">
  {#if section === 'home'}
    <section class="hero container">
      <div class="hero-copy"><p class="eyebrow"><span class="square-dot" aria-hidden="true"></span> {config.brand.tagline}</p><h1><span>{copy.headline[0]}</span><em>{copy.headline[1]}</em></h1><p class="lead">{copy.lead.replaceAll('AdLinker',config.brand.name)}</p>
        <div class="actions"><a class="button primary" href={register || '#how-it-works'}>{register ? copy.start : copy.primary}<span aria-hidden="true">↗</span></a><a class="button secondary" href={link('calculator')}><svg aria-hidden="true" viewBox="0 0 24 24"><rect x="5" y="3" width="14" height="18" rx="2"/><path d="M8 7h8M8 11h2m4 0h2M8 15h2m4 0h2"/></svg>{copy.secondary}</a></div>
        <div class="hero-notes">{#each copy.heroNotes as note}<span><b aria-hidden="true">✓</b>{note}</span>{/each}</div>
      </div>
      <FlowCard {copy} />
    </section>
    ...
    <section class="section feature-section"><div class="container"><div class="section-heading"><div><p class="eyebrow">02 / {config.brand.name}</p><h2>{copy.featuresTitle}</h2></div><p>{copy.featuresLead}</p></div><div class="features-grid">{#each copy.features as feature, i}<article class="feature-item"><span class="feature-icon" aria-hidden="true">{['↗','⊞','≋','↶','▥','◇'][i]}</span><h3>{feature[0]}</h3><p>{feature[1]}</p></article>{/each}</div></div></section>
    <section class="section dark-section"><div class="container path-layout"><div><p class="eyebrow">03 / {copy.howLabel}</p><h2>{copy.pathTitle}</h2><p class="section-description">{copy.pathLead}</p><a class="text-link" href={link('why')}>{copy.nav[0]} <span aria-hidden="true">↗</span></a></div><div class="path-options">{#each copy.paths as path, i}<article><span class="path-number">0{i+1}</span><div><h3>{path[0]}</h3><p>{path[1]}</p></div><span aria-hidden="true">↗</span></article>{/each}</div></div></section>
    <section class="section"><div class="container pricing-block"><div><p class="eyebrow">04 / {copy.estimate}</p><h2>{copy.pricingHeading}</h2><p>{copy.pricingText}</p><a class="button primary" href={link('calculator')}>{copy.secondary}<span aria-hidden="true">↗</span></a></div><div class="price-composition">{#each copy.priceParts as part, i}<div><span class="composition-icon" aria-hidden="true">{['▤','⊞','↗'][i]}</span><span>{part}</span><span aria-hidden="true">{i < 2 ? '+' : '='}</span></div>{/each}<p>{copy.calc.total}<svg aria-hidden="true" viewBox="0 0 80 30"><path d="M2 25h15l9-9h14l12-10h23m-10-4h10v10"/></svg></p></div></div></section>
    <section class="section faq-section"><div class="container faq-layout"><div><p class="eyebrow">FAQ</p><h2>{copy.faqTitle}</h2></div><div>{#each copy.faq as item}<details class="faq-item"><summary>{item[0]}<span aria-hidden="true">+</span></summary><p>{item[1]}</p></details>{/each}</div></div></section>
  {:else if section === 'why'}
  ...
  {:else if section === 'tiktok-events-api-whatsapp'}
    <section class="page-hero container"><p class="eyebrow">TikTok Events API</p><h1>{lang === 'zh-hans' ? 'TikTok WhatsApp 事件回传说明' : 'TikTok Events API for WhatsApp Leads'}</h1><p class="lead">{lang === 'zh-hans' ? '了解事件状态、发送边界，以及为什么发送成功不等于平台归因。' : 'Understand event states, delivery boundaries and why a sent event is not the same as platform attribution.'}</p></section>

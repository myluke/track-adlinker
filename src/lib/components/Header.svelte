  ...
  <div class="container nav-wrap">
    ...
    <div class="tools">
      <details class="language-switch">
        <summary aria-label={copy.language}><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><ellipse cx="12" cy="12" rx="4" ry="9"/><path d="M3 12h18"/></svg><span>{locales.find((l) => l.code === lang)?.label}</span><span class="chevron">⌄</span></summary>
        <div class="language-options">{#each locales as locale}<a href={link(locale.code)} lang={locale.hreflang} hreflang={locale.hreflang} aria-current={locale.code === lang ? 'true' : undefined} data-sveltekit-reload>{locale.label}{#if locale.code === lang}<span aria-hidden="true">✓</span>{/if}</a>{/each}</div>
      </details>
      ...
      {#if register}<a class="button primary compact header-cta" href={register}>{copy.start} <span aria-hidden="true">↗</span></a>{/if}

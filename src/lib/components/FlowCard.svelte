...
<figure class="journey-figure">
  <div class="journey-board">
    <div class="board-head"><span class="eyebrow">{copy.diagramMeta}</span><span class="board-mark" aria-hidden="true"><Icon name="arrow-up-right" /></span></div>
    ...
    <div class="journey-map">
      ...
      <button class="feedback-chip" class:active={active === 3} onclick={() => active = 3} aria-pressed={active === 3}><Icon name="undo" /> {copy.stages[3]}</button>
      <span class="signal-dot first" aria-hidden="true"></span><span class="signal-dot second" aria-hidden="true"></span>
    </div>
    <div class="journey-detail" aria-live="polite"><span class="detail-number">0{active + 1}</span><div><strong>{copy.stages[active]}</strong><p>{copy.stageNotes[active]}</p></div><span class="detail-check" aria-hidden="true"><Icon name="check" /></span></div>
  </div>
